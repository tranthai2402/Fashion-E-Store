...
function MenuItems() {
  ...
  return (
    <nav className="relative flex items-center gap-[6px]">
      <div
        className="absolute top-0 left-0 flex h-[30px] items-center rounded-[6px] bg-[#0e0f1114] transition-all duration-300 ease-out"
        style={{
          ...hoverStyle,
      ...
      {shoppingViewHeaderMenuItems.map((menuItem, index) => (
        <button
          ...
          className={`z-10 h-[30px] px-3 py-2 rounded-md bg-transparent border-none cursor-pointer text-[11px] font-medium uppercase tracking-[0.2em] transition-colors duration-300 outline-none ${
            activeId === menuItem.id ? "text-black" : "text-gray-400"
          }`}
        >
          {menuItem.label}
        </button>
    ...
    <div className="flex items-center gap-5">
      ...
      <Sheet open={openCartSheet} onOpenChange={setOpenCartSheet}>
        <button
          ...
          className="relative text-gray-600 hover:text-black transition-colors bg-transparent border-none cursor-pointer"
        >
          <ShoppingCart className="w-[18px] h-[18px]" strokeWidth={1.5} />
          {cartItems?.items?.length > 0 && (
            <span className="absolute -top-2 -right-2 bg-black text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-medium">
              {cartItems.items.length}
            </span>
          )}
        </button>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="bg-transparent border-none cursor-pointer">
            <Avatar className="w-7 h-7 bg-black">
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-sm border-b border-gray-100">
      <div className="flex h-14 items-center justify-between px-6 md:px-10 w-full">
        ...
        >
          <span className="text-xl font-bold tracking-[0.05em] text-black uppercase">
            Saint Laurent
          </span>
        </Link>
